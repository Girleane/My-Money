...
class DespesaHelper {
  ...
  Future<Database> initDb() async {
    ...
    return await openDatabase(path,version: 1,onCreate: (Database db,int newerVersion) async {
      await db.execute(
        "CREATE TABLE $despesaTable($idColumn INTEGER PRIMARY KEY, $titleColumn TEXT, $subtitleColumn TEXT, $valueColumn FLOAT)"
      );
    });
  }
  ...
  Future<Despesa?> getDespesa(int id) async {
    Database? dbDespesa = await db;
    List<Map> maps = await dbDespesa!.query(despesaTable, columns: [idColumn, titleColumn, subtitleColumn, valueColumn], where: "$idColumn = ?",whereArgs: [id]);
    if(maps.length > 0){
  ...
  Future<int> updateDespesa(Despesa despesa) async {
    Database? dbDespesa = await db;
    return await dbDespesa!.update(despesaTable,despesa.toMap(), where: "$idColumn = ?",whereArgs: [despesa.id]);
  }
  ...
  Future<List> getDespesasPorMes(String data) async {
    Database? dbDespesa = await db;
    List listMap = await dbDespesa!.rawQuery("SELECT * FROM $despesaTable WHERE $subtitleColumn LIKE '%$data%'");
    List<Despesa> listDespesa = [];
    for (Map map in listMap) {
      listDespesa.add(Despesa.fromMap(map));
    }
  ...
  Future<int?> getNumber() async {
    Database? dbDespesa = await db;
    return Sqflite.firstIntValue(await dbDespesa!.rawQuery("SELECT COUNT(*) FROM $despesaTable"));
  }
...
class Despesa {
  ...
  @override
  String toString() {
    return "Despesa:(id:$id,title:$title,subtitle:$subtitle,value:$value)";
  }
}